...
class CTransManager:
    ...
    def create_ctransfile(self, srcpath, outdir):
        ...
        if srcpath.is_dir():
            ...
            srcpath_name = srcpath.name
            for k, v in self.replace_dict.items():
                pattern = "^"+k+"$"
                result = re.match(pattern, srcpath_name)
                if result != None:
                    srcpath_name = srcpath_name.replace(k, v)
            transdir = os.path.join(outdir, srcpath_name)

            # Execute child paths recursively
            for child_path in sorted(srcpath.iterdir()):
                self.create_ctransfile(child_path, transdir)

        # If srcpath is a file (not a directory)
        else:
            ...
            srcpath_root, srcpath_ext = os.path.splitext(srcpath)
            if not srcpath_ext in self.extentions:
                return

            os.makedirs(outdir, exist_ok=True)
            ctrans_path = os.path.join(outdir, srcpath)
            if not os.path.isfile(ctrans_path):
                ctransfile = CTransFile()
                ctransfile.create(srcpath, outdir)
    ...
    def create_ctranslation_file(self, srcpath, transdir, outdir):
        ...
        if srcpath.is_dir():
            ...
            # Replace directories when speficied -r option.
            curr_srcpath = srcpath.name
            for k, v in self.replace_dict.items():
                pattern = "^"+k+"$"
                result = re.match(pattern, curr_srcpath)
                if result != None:
                    curr_srcpath = curr_srcpath.replace(k, v)

            curr_transdir = os.path.join(transdir, curr_srcpath)
            curr_outdir   = os.path.join(outdir, srcpath.name)

            # Execute this method for child paths recursively
    ...
    def output_lines(self, srcfile, ctransfile, outfile):
        ...
        if start_token_pos != -1:
            ...
            # Get first source part in ctranslation file
            line_ctrans = ctransfile.get_current_line()

        # Get one line from source file
        line_src = srcfile.get_current_line()
        old_line_src_pos = srcfile.get_curr_line_num()

        # In case srcfile may have no contents
        if srcfile.status == srcfile.STATUS_NORMAL:
            return 0;
        ...
        while srcfile.get_curr_line_num() <= srcfile.get_line_count() and proc_continue:
            ...
            if srcfile.is_error():
                if ctransfile.is_normal() and line_ctrans != "":
                    ...
                    while True:
                        ...
                        if result == None: # and line_src != line_ctrans:
